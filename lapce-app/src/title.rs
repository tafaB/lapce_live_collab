...
use floem::{
    ...
    view::View,
    views::{container, stack, Decorators, Label},
    views::{label, svg, text_input},
    ViewContext,
...
fn left(
    ...
    stack(move || {
        (
            //LAPCE LIVE COLLAB -------------------------
            container(move || {
                svg(move || config.get().ui_svg(LapceIcons::COLLAB)).style(
                    move || {
            ...
            .on_click(move |_| {
                ...
                let mut menu = Menu::new("").entry(
                    MenuItem::new("GO COLLAB PANEL").action(move || {
                        workbench_command
                            .send(LapceWorkbenchCommand::ToggleCollabVisual);
                    }),
                );
            ...
            .style(move || {
                Style::BASE
                    ...
                    .items_center()
                    .background(
                        *config.get().get_color(LapceColor::LAPCE_COLLAB),
                    )
            }),
            //LAPCE LIVE COLLAB -------------------------
            container(move || {
